...
class MatchService(
    ...
    private fun getMatchRecommendationsWithCache(
        ...
        val filteredCandidates = filterCachedCandidates(
            cachedCandidates, sender.id, sender.gender, sender.university,
            sleepPatternFilter, ageRangeFilter, cleaningFrequencyFilter, startDate, endDate
        )

        val recommendations = buildCachedRecommendations(filteredCandidates, senderPreference)
    ...
    ): List<MatchRecommendationResponse.MatchRecommendationItem> {
        return candidates
    ...
    private fun buildCachedRecommendationItem(
        ...
        senderPreference: UserMatchPreference
    ): MatchRecommendationResponse.MatchRecommendationItem {
        ...
        val existingMatch = matchRepository.findBySenderIdAndReceiverId(
            senderPreference.user.id, candidate.userId
        )

        val matchType = existingMatch.map { it.matchType }.orElse(MatchType.NONE)
        val matchStatus = existingMatch.map { it.matchStatus }.orElse(MatchStatus.NONE)

        // 디버깅 로그
        log.info(
            "매칭 상태 조회 - senderId: {}, receiverId: {}, matchType: {}, matchStatus: {}",
            senderPreference.user.id, candidate.userId, matchType, matchStatus
        )
        ...
        return MatchRecommendationResponse.MatchRecommendationItem.builder()
            ...
            .studentVerified(candidate.studentVerified)
            .gender(candidate.gender)
            .age(matchUtilityService.calculateAge(candidate.birthDate))
            .mbti(candidate.mbti)
            .preferenceScore(similarityScore)
            .matchType(matchType)
            .matchStatus(matchStatus)
            // 추가 프로필 정보
            .sleepTime(candidate.sleepTime)
            .cleaningFrequency(candidate.cleaningFrequency)
            .isSmoker(candidate.isSmoker)
            .startUseDate(candidate.startUseDate?.toString())
            .endUseDate(candidate.endUseDate?.toString())
            .build()
    }
    ...
    private fun getMatchRecommendationDetailWithCache(
        ...
        val existingMatch = matchRepository.findBySenderIdAndReceiverId(sender.id, receiverId)

        val matchType = existingMatch.map { it.matchType }.orElse(MatchType.NONE)
        val matchStatus = existingMatch.map { it.matchStatus }.orElse(MatchStatus.NONE)

        return MatchRecommendationDetailResponse.builder()
            .receiverId(cachedReceiver.userId)
            .email(cachedReceiver.email)  // 신고 기능을 위한 이메일 추가
            .name(cachedReceiver.name)
            .university(cachedReceiver.university)
            .studentVerified(cachedReceiver.studentVerified)
            .mbti(cachedReceiver.mbti)
            .gender(cachedReceiver.gender)
            .age(matchUtilityService.calculateAge(cachedReceiver.birthDate))
            .isSmoker(cachedReceiver.isSmoker)
            .isPetAllowed(cachedReceiver.isPetAllowed)
            .isSnoring(cachedReceiver.isSnoring)
            .sleepTime(cachedReceiver.sleepTime)
            .cleaningFrequency(cachedReceiver.cleaningFrequency)
            .hygieneLevel(cachedReceiver.hygieneLevel)
            .noiseSensitivity(cachedReceiver.noiseSensitivity)
            .drinkingFrequency(cachedReceiver.drinkingFrequency)
            .guestFrequency(cachedReceiver.guestFrequency)
            .preferredAgeGap(cachedReceiver.preferredAgeGap)
            .birthDate(cachedReceiver.birthDate)
            .startUseDate(cachedReceiver.startUseDate)
            .endUseDate(cachedReceiver.endUseDate)
            .preferenceScore(similarityScore)
            .matchType(matchType)
            .matchStatus(matchStatus)
            .build()
    ...
    fun confirmMatch(matchId: Long, userId: Long): Match {
        ...
        // 매칭 선호도 등록 여부 확인
        validateUserMatchPreference(match.sender.id)
        validateUserMatchPreference(match.receiver.id)
    ...
    fun getMatchStatus(userId: Long): MatchStatusResponse {
        val matches = matchRepository.findBySenderIdOrReceiverId(userId)

        val matchItems = matches.map { match ->
            matchUtilityService.toMatchStatusItem(match, userId)
        }
        ...
        val summary = MatchStatusResponse.SummaryInfo.builder()
            .total(total)
            .pending(pending)
            .accepted(accepted)
            .rejected(rejected)
    ...
    fun getMatchResults(userId: Long): MatchResultResponse {
        val results = matchRepository.findBySenderIdOrReceiverId(userId)
    ...
    fun sendLike(requestDto: LikeRequest, senderId: Long): LikeResponse {
        ...
        if (existingMatchOpt.isPresent) {
            ...
            if (existingMatch.sender.id == senderId) {
            ...
            return LikeResponse(existingMatch.id, true)
        } else {
            ...
            // 기존 기록이 없는 경우 (처음 '좋아요')
            val newLike = Match.createLike(sender, receiver, preferenceScore)
            ...
            return LikeResponse(newLike.id, false) // 아직 상호 매칭(요청)은 아님
        }
    }
    ...
    private fun validateMatchParticipant(match: Match, userId: Long) {
        if (match.sender.id != userId && match.receiver.id != userId) {
    ...
    private fun validateAndHandleMatchTypeTransition(match: Match) {
        when {
            match.matchType == MatchType.LIKE -> {
